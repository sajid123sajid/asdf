...
import { Camera, ChevronDown, Heart, LayoutGrid, MapPin, Search, ShoppingCart, Smartphone, User, Zap } from "lucide-react";
...
export function Header() {
  ...
  const collapsed = isMobile && scrolledDown;
  ...
  return (
    <header
      className="sticky top-0 z-50 border-b border-border/70 bg-card/95 backdrop-blur-sm transition-transform duration-300 ease-out will-change-transform"
      style={collapsed && shift > 0 ? { transform: `translateY(-${shift}px)` } : undefined}
    >
      <div className="hidden border-b border-border/70 bg-[#f7f4ef] md:block">
      ...
      <div className="mx-auto max-w-[1200px] px-3 py-3 sm:px-4 md:py-4">
        <div className={`flex items-center gap-3 transition-opacity duration-200 ease-out ${collapsed ? "opacity-0" : "opacity-100"}`}>
          ...
          <div className="hidden flex-1 justify-center md:flex md:px-6">
            <form
              ref={searchRef}
              className="flex h-12 w-full max-w-[680px] items-center overflow-hidden rounded-full border border-border bg-secondary/80 shadow-sm ring-1 ring-transparent transition focus-within:border-gold focus-within:ring-gold/20"
              onSubmit={(e) => {
                e.preventDefault();
                const query = q.trim();
            ...
            >
              <Search className="ml-4 h-4 w-4 shrink-0 text-gold" />
              <input
                type="search"
                value={q}
              ...
              <button
                type="submit"
                aria-label="Search"
                className="flex h-12 w-16 items-center justify-center bg-gold text-primary-foreground transition-colors hover:bg-gold-deep"
              >
          ...
          <div className="ml-auto hidden items-center gap-5 md:flex">
            <Link to="/wishlist" className="flex items-center gap-2 rounded-full border border-border bg-secondary px-3 py-2 text-xs font-medium text-foreground transition-colors hover:border-gold/40 hover:text-gold" activeProps={{ className: "border-gold/50 text-gold" }}>
            ...
            <Link to="/account" className="flex items-center gap-2 rounded-full border border-border bg-secondary px-3 py-2 text-xs font-medium text-foreground transition-colors hover:border-gold/40 hover:text-gold" activeProps={{ className: "border-gold/50 text-gold" }}>
              <User className="h-4 w-4" />
              <span>Account</span>
            </Link>
            <button type="button" onClick={openCart} aria-label="Open cart" className="relative flex items-center gap-2 rounded-full border border-border bg-secondary px-3 py-2 text-xs font-medium text-foreground transition-colors hover:border-gold/40 hover:text-gold">
